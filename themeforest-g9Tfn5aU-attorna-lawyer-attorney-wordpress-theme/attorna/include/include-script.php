<?php 
	...
	*/	
	...
	if( !function_exists('attorna_get_style_custom') ){
		function attorna_get_style_custom($local = false){

			$upload_dir = wp_upload_dir();
			$filename = '/attorna-style-custom.css';
	...
	add_action( 'wp_enqueue_scripts', 'attorna_include_scripts', 11 );
	if( !function_exists('attorna_include_scripts') ){
		function attorna_include_scripts(){
			...
			// include site style
			wp_enqueue_style('attorna-style-core', get_template_directory_uri() . '/css/style-core.css');
			if( is_rtl() ){
				wp_enqueue_style('frontend-rtl', get_template_directory_uri() . '/css/frontend-rtl.css');
			}
			
			if( !is_customize_preview() ){
				wp_enqueue_style('attorna-custom-style', attorna_get_style_custom());
			}
			...
			wp_enqueue_script('attorna-script-core', get_template_directory_uri() . '/js/script-core.js', array('jquery', 'jquery-effects-core'), '1.0.0', true );
	...
	add_action( 'wp_enqueue_scripts', 'attorna_include_child_scripts', 999 );
	if( !function_exists('attorna_include_child_scripts') ){
		function attorna_include_child_scripts(){
			if( is_child_theme() ){
				wp_enqueue_style('attorna-child-theme-style', get_stylesheet_uri());
			}
		}
	}